...
--*/
{
    ...
    if (NT_SUCCESS(ntStatus))
    {
        ...
        if (!NT_SUCCESS(Port_->QueryInterface(IID_IDrmPort, (PVOID *) &m_pDrmPort)))
        {
            m_pDrmPort = nullptr;
        }
    }
...
--*/
{
    ...
    if (ulContentIndex)
    {
        ntStatus = m_pDrmPort->CreateContentMixed(ulContentIds, ulContentIndex, &ulMixDrmContentId);
        if (NT_SUCCESS(ntStatus))
        {
            fCreatedContentId = TRUE;
            ntStatus = m_pDrmPort->GetContentRights(ulMixDrmContentId, &MixDrmRights);
        }
    }
...
*/
{
    ...
    const ULONG oldContentId = contentId;

    m_ulContentId = contentId;

    // Miniport should create a mixed DrmRights.
    //
    NTSTATUS ntStatus = m_pMiniportLocal->UpdateDrmRights();

    // Restore the old content Id.
    //
    if (!NT_SUCCESS(ntStatus))
    {
        m_ulContentId = oldContentId;
    }

    // MSVAD rights each stream separately to disk. If the rights for this
